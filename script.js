...
function addItem() {
  ...
  if (imageFile) {
    ...
    reader.onload = function(e) {
      saveItem({ name, type, colorHex, colorName, imgData: e.target.result });
    };
    reader.readAsDataURL(imageFile);
  } else {
    saveItem({ name, type, colorHex, colorName, imgData: null });
  }
}
...
function saveEdit() {
  ...
  if(imageFile){
    let reader=new FileReader();
    reader.onload=function(e){updateItem(e.target.result);}
    reader.readAsDataURL(imageFile);
  } else { updateItem(); }
}
...
function showFavorites() { 
  applyFilters(wardrobe.filter(item=>item.favorite)); 
}
...
function toggleView(){
  ...
  if(isGridView){
    ...
    event.target.innerText="Switch to Grid View";
  }
}
